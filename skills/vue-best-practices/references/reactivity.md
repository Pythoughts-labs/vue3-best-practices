...
Vue 3.5+ also exposes `onWatcherCleanup()` (imported from `vue`), useful when the cleanup is registered from a nested helper that does not receive the `onCleanup` argument. Register it synchronously, before any `await`.
